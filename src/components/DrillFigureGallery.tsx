...
import firmes from '@/assets/drill/firmes.jpg';
import descanso from '@/assets/drill/descanso.jpg';
import saludo from '@/assets/drill/saludo.jpg';
import flanco from '@/assets/drill/flanco.jpg';
import marcha from '@/assets/drill/marcha.jpg';

type PosKey = 'firmes' | 'descanso' | 'saludo' | 'flanco' | 'marcha';

const order: PosKey[] = ['firmes', 'descanso', 'saludo', 'flanco', 'marcha'];
const images: Record<PosKey, string> = { firmes, descanso, saludo, flanco, marcha };

// Map each voice-command example index to a target position
const exampleToPosition: Record<number, PosKey> = {
  0: 'marcha',  // De frente… ¡MAR!
  1: 'firmes',  // Atención… ¡FIR!
  2: 'flanco',  // Flanco derecho… ¡YA!
  3: 'firmes',  // Alto… ¡ALTO!
};

const DrillFigureGallery = () => {
  const { t } = useI18n();
  const [current, setCurrent] = useState<PosKey>('firmes');
